...
# # Check Security Feeds from Open Source APIs
# @login_required
# def check_security_feeds(request):
#     threat_url = "https://www.threatcrowd.org/searchApi/v2/domain/report/"
#     email_url = request.POST.get('email_url', '')
#     domain = request.POST.get('domain', '')
    
#     # Fetching threat intelligence data
#     response = requests.get(threat_url, params={"domain": domain})
#     threat_data = response.json()
    
#     # Checking email URL security
#     phish_response = requests.get(f'https://phishious.caniphish.com/api/analyze?url={email_url}')
#     phishing_data = phish_response.json()
    
#     return JsonResponse({"Threat Intelligence": threat_data, "Phishing Data": phishing_data})
